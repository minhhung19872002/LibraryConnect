...
describe('Phạm vi dữ liệu của người dùng', () => {
  ...
  it('tách phạm vi máy chủ trả về thành hai ô, bỏ qua kiểu khác', () => {
    expect(
      splitDataScopes([
        { scopeType: 'Library', scopeId: 'L1' },
    ...
    ).toEqual({ libraryIds: ['L1'], warehouseIds: ['W1'] });

    expect(splitDataScopes(undefined)).toEqual({ libraryIds: [], warehouseIds: [] });
  });

  it('đi vòng: dựng rồi tách ra đúng hai ô ban đầu', () => {
    const values = { libraryIds: ['L1', 'L2'], warehouseIds: ['W1'] };

...
(L9, L10, L11), trong đó hai lỗi tranh chấp là loại chỉ hiện ra khi gửi yêu cầu thật sự song song.
Đợt thứ mười hai đi theo ba câu hỏi nữa — **endpoint nào chưa được canh quyền**, **bất biến dữ liệu
trên chính kho thật** (38 phép đo SQL), và **việc chạy nền theo lịch có thật sự chạy không** (12 phép
đo) — thêm **3 lỗi** (L14, L15, L16). Chín việc nền đều chạy đúng giờ, 15 lượt thành công, 0 lượt hỏng;
26/29 bất biến sạch ngay từ đầu, ba cờ đỏ còn lại dẫn thẳng tới L15 và L16. Kết quả từng phép đo ở phụ lục cuối
...
| L9 | Toàn bộ API — mọi ô nhập chữ | **Một ký tự rỗng U+0000 trong dữ liệu gửi lên làm bảy endpoint trả 500**, trong đó có ba lối công khai không cần đăng nhập. Cột kiểu text của PostgreSQL không chứa được ký tự này: máy chủ trả `22021: invalid byte sequence for encoding "UTF8": 0x00` ở tầng trình điều khiển — sau khi câu truy vấn đã gửi đi, nên không bộ kiểm tra nào của tầng nghiệp vụ chặn kịp. `PlainText.RemoveUnstorableCharacters` đã có sẵn từ trước và làm đúng việc ấy, nhưng chỉ được gọi ở một đường duy nhất: lớp chữ rút từ tệp PDF. | `GET /api/search?keyword=gi%C3%A1o%00tr%C3%ACnh` → 500. Cùng cách ấy: `/search/suggest`, `/search/facets`, `/public/news`, `/cataloging/bibs`, `/readers`, `/admin/audit-logs` — bảy endpoint. Lối ghi thì trả 409 "Không lưu được dữ liệu", không nói được sai ở đâu. | **Nặng** | Bảo mật | Đã sửa — lọc ở đúng hai cửa mà chuỗi vào hệ thống: lớp trung gian cho chuỗi truy vấn, bộ chuyển đổi JSON cho thân yêu cầu; `UnstorableTextTests` (9 phép thử, 8 đỏ trước khi sửa) |
| L10 | Bạn đọc → Cấp lại thẻ | **Ba lượt cấp lại thẻ bấm cùng lúc để lại ba thẻ cùng hiệu lực.** Tầng nghiệp vụ hạ cờ thẻ cũ rồi mới dựng thẻ mới — đọc-rồi-ghi, không có ràng buộc duy nhất ở cơ sở dữ liệu. Hậu quả nghiệp vụ: thẻ vừa báo mất vẫn quét được ở cổng và ở quầy, đúng thứ chức năng "cấp lại thẻ" sinh ra để ngăn. Phép thử tuần tự (NV.29) vẫn xanh vì gọi lần lượt. | Ba lượt `POST /api/readers/{id}/cards/reissue` song song trên máy chủ thật: cả ba trả 200; `select count(*) from rdr.reader_cards where reader_id=… and is_current` = **3**. | **Nặng** | Nghiệp vụ | Đã sửa — chỉ mục duy nhất riêng phần `ux_reader_cards_hien_hanh`, kèm migration dọn thẻ trùng trên bản đã cài (giữ thẻ mới nhất); `ConcurrencyTests.Ba_luot_cap_lai_the_cung_luc…` |
| L11 | Bổ sung → Kiểm kê | **Ba lượt mở kỳ kiểm kê cùng một kho, bấm cùng lúc, đều thành công.** Bộ xử lý có hỏi "kho này còn kỳ chưa chốt không" nhưng đó cũng là đọc-rồi-ghi. Kho có ba kỳ đang mở, mỗi kỳ chốt một danh sách kỳ vọng riêng, nên cán bộ quét vào kỳ nào cũng ra kết quả sai — và danh sách thiếu là căn cứ lập quyết định mất (III.4 bước 5). | Ba lượt `POST /api/inventory/periods` song song cùng `warehouseId`: mã trả về `[200, 200, 200]`, kho có 3 kỳ `InProgress`. | **Nặng** | Nghiệp vụ | Đã sửa — chỉ mục duy nhất riêng phần `ux_inventory_periods_kho_chua_chot`, kèm migration dọn kỳ dư (kỳ chưa ai quét thì xoá mềm, kỳ đã có lượt quét thì chốt lại để giữ số liệu); `ConcurrencyTests.Ba_luot_mo_ky_kiem_ke…` |

| L14 | Toàn hệ thống — `GET /api/staff/options` | **Thẻ đăng nhập của bạn đọc đọc được danh sách cán bộ kèm tên đăng nhập.** Endpoint chỉ có `[Authorize]`, không có `[RequirePermission]` và cũng không tự hỏi tài khoản đang gọi có phải cán bộ không — mà thẻ bạn đọc thì qua `[Authorize]` như thường. Chính lời chú của lớp ấy viết "chỉ đòi đăng nhập bằng tài khoản cán bộ", nhưng không có gì bắt điều đó. Tên đăng nhập là thứ cần có trước khi đi dò mật khẩu; ai có thẻ thư viện đều lấy được. Chuông thông báo ngay bên cạnh thì chặn đúng từ đầu. | Lập một bạn đọc mới, đăng nhập, gọi `GET /api/staff/options` → **200** kèm `[{"id":…,"fullName":"Quản trị hệ thống","username":"admin"}]`. | Vừa | Bảo mật | Đã sửa — bộ xử lý đòi `UserId` của cán bộ, đúng cách chuông thông báo đang làm. Kèm phép thử quét `EndpointAuthorisationTests` buộc **mọi** endpoint quản trị phải khai `[RequirePermission]`, khai `[AllowAnonymous]`, hoặc ghi tên vào danh sách "tự canh trong bộ xử lý" kèm lý do |
| L15 | Biên mục → Hàng đợi biên mục | **Hàng đợi đếm cả việc mà nó không hiện nổi.** Xóa biểu ghi không đụng tới dòng việc trong hàng đợi, nên việc ấy nằm lại; phần **đếm** chạy thẳng trên bảng công việc, còn phần **lấy dòng** phải nối sang biểu ghi để lấy nhan đề nên bị bộ lọc xóa mềm gạt bớt. Hai con số không bao giờ gặp nhau, và trang cuối rỗng trơn. | Trên máy chủ thật: `GET /api/cataloging/queue?status=Pending&pageSize=200` báo `totalCount = 981` nhưng trả về **155** dòng; trang 5 trả 181, trang 6 trả 0. Đếm trong CSDL: đúng **45** dòng việc trỏ tới biểu ghi đã xóa. | Vừa | Nghiệp vụ | Đã sửa — xóa biểu ghi thì xóa luôn dòng việc của nó; cả danh sách lẫn ba bộ đếm chỉ tính việc của biểu ghi còn sống; kèm migration dọn 45 dòng cũ. `CatalogQueueTests.Xoa_bieu_ghi_thi_viec_bien_muc…` |
| L16 | Bổ sung → Xóa ĐKCB · lịch sử mượn của bạn đọc | **Xóa một bản sách đã trả xong làm mất luôn lượt mượn ấy khỏi lịch sử.** Phiếu mượn nối sang ĐKCB bằng điều hướng bắt buộc, nên mọi phép chiếu có nhan đề, ký hiệu xếp giá hay tên kho đều rơi cả dòng phiếu khi bản sách bị xóa mềm — lịch sử bạn đọc mất dòng, danh sách phiếu ở quầy mất dòng, và khoản phạt gắn phiếu ấy biến khỏi danh sách phạt trong khi vẫn tính vào công nợ. Lối xóa chỉ chặn khi bản **đang** được mượn. Đây là bài học 57 lần thứ tư. | Trên máy chủ thật, bạn đọc TV2026000489 có 5 phiếu trong CSDL; `GET /api/readers/{id}/loans` báo `totalCount = 5` mà chỉ trả về **4 dòng** — thiếu đúng phiếu của bản sách đã xóa. | Vừa | Nghiệp vụ | Đã sửa — bản đã có lượt mượn thì không xóa được nữa, câu chặn chỉ sang chức năng thanh lý (thứ giữ được lịch sử); lịch sử bạn đọc lấy nhan đề bằng câu hỏi phụ chứ không qua điều hướng bắt buộc, nên dòng đã lỡ mồ côi vẫn hiện. `CirculationTests.Ban_sach_da_tung_luu_thong…` |
...
## S. Đợt rà thứ mười chín — ứng dụng di động, quét ngang từng màn hình (08/09/2026)

Mười tám đợt trước quét ngang phía web sáu lần — danh sách, ô lọc, báo cáo, chiều ghi, tệp xuất,
canh quyền. **Ứng dụng di động thì chưa lần nào**: nó mới được soi ở hai lớp gián tiếp — đợt J đọc
đặc tả phân hệ XI, đợt P soi nhóm `/api/reader/*` ở phía máy chủ. Bản thân mã Flutter chưa ai đo.

Đợt này lấy đúng một dòng của đặc tả làm luật, mục XI.3: *"Hỗ trợ sáng/tối, cỡ chữ điều chỉnh
được"*, cộng một luật của mục 6.6 (*"contrast đạt WCAG AA"*). Ba câu hỏi cho từng màn hình:

> 1. Dựng ở cỡ chữ 100%, 150%, 200% có tràn khung không?
> 2. Ở chế độ tối, chữ có đọc được trên nền của chính nó không?
> 3. Mọi danh sách có nói được cả ba trạng thái — đang tải, rỗng, lỗi?
...
| S6 | Tài khoản → hàng "Giao diện" và hàng "Ngôn ngữ" (XI.3) | **Nhãn bị ô chọn bóp đến mức vỡ dòng giữa từ.** Ô chọn giao diện đặt ở cột `trailing` của `ListTile`; ở cỡ chữ lớn nó ăn gần hết bề ngang màn hình điện thoại, nhãn còn hơn bốn chục điểm ảnh và "Giao diện" xuống dòng thành **"Gi / ao / diệ / n"**. Không ngoại lệ tràn khung nào được ném — hàng vẫn "vừa", chỉ là không đọc được nữa. Hàng "Cỡ chữ" ngay dưới vốn đã đặt thanh trượt ở `subtitle`, tức cách đúng đã có sẵn ngay cạnh. | Ảnh `ios-13-toi-tai-khoan.png` do máy Mac của GitHub chụp trên iPhone Simulator ở cỡ chữ 160%. Đo lại trong phép thử với khung dựng bằng màn hình điện thoại: ô chữ **rộng 42 cao 152** ở 160%, và **rộng 0 cao 384** ở 200%. Nới phép đo sang hàng kế bên thì hàng "Ngôn ngữ" cũng vỡ ở 200%: **rộng 63 cao 144**. | Vừa | Giao diện | Đã sửa — ô chọn của **cả hai** hàng xuống `subtitle`, cùng cách với hàng "Cỡ chữ" ở giữa; `account_text_scale_test.dart` đo bất biến "chữ ngắn thì ô rộng hơn cao" cho từng nhãn ở ba cỡ (đỏ trước khi sửa) |
...
- **Phông vẫn đóng gói trong ứng dụng**: `GoogleFonts.config.allowRuntimeFetching = false` còn
  nguyên trong `main.dart` — bài học 27 không quay lại.
- **Chỉ 3 mã màu viết tho** trong toàn bộ mã màn hình, cả ba nằm trên dải màu đầu trang chủ vốn tối
  ở cả hai chế độ.
...
## T. Đợt rà thứ hai mươi — giao diện quản trị ở đúng khổ màn hình đã cam kết (08/09/2026)

Đợt 19 để lại một bài học chưa ai áp sang phía web: **khung đo rộng hơn thiết bị thật thì bố cục bị
bóp vẫn "vừa"** (bài học 98). Ở di động, phép thử dựng ở 800×600 nên không thấy nhãn vỡ dòng. Câu
hỏi tương ứng ở đây lấy thẳng từ mục 6.6:

> *"Responsive: admin tối thiểu 1366×768"* — mà **mọi ảnh chụp của mười chín đợt trước đều ở
> 1440×900**, rộng hơn mức cam kết 74 điểm ảnh. Ở đúng khổ tối thiểu ấy, màn hình nào vỡ?

Luật đo: ở 1366×768, **trang không được cuộn ngang**. Bảng rộng thì cuộn trong khung của nó — điều
sản phẩm vốn đã làm và còn có dòng nhắc riêng (lỗi B10) — nhưng đẩy cả trang thì thanh cuộn ngang
...
- **67 phép đo theo thẻ**: bấm qua từng thẻ của 41 màn hình có thẻ (báo cáo bổ sung 6 thẻ, lưu thông,
  bạn đọc, tài liệu số, ấn phẩm định kỳ, tham số hệ thống, sao lưu…) — chỉ thẻ "Tổng quát" của báo
  cáo bổ sung vỡ.
- **Bảng rộng vẫn cuộn trong khung của nó**, không đẩy trang: đúng cách sản phẩm đã chọn từ lỗi B10.
- **Luật bàn phím của mục 6.6**: quét toàn bộ mã giao diện quản trị — **0 phần tử `div`/`span` gắn
  `onClick`**, tức không có chỗ nào bấm được bằng chuột mà bàn phím không tới. Luật này trước nay chỉ
